...
0. **Freshness Preflight.** Before treating the input as authoritative Navigation context, check whether the context is fresh enough to map routes from.
   ...
   - `refresh_needed`: load `cognitive_harness/navigation/warmup/navigator-refresh.md` in full and execute it before producing the Navigation map. Use the produced sync brief as additional input, and record the sync brief path or inline brief in the Navigation output.
