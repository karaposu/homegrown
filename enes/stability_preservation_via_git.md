...
Git already preserves every prior state of every file. But disciplines are runtime artifacts: they live in `~/.claude/skills/<name>/` and are dispatched by slash command. Reading `git show bf4ae1f:homegrown/sense-making/SKILL.md` shows you the file but does not give you a callable `/sense-making@bf4ae1f`. The slash command always resolves to whatever is currently installed.

A naïve `git checkout bf4ae1f -- homegrown/` would replace current `homegrown/`, eliminating the comparison rather than enabling it. Branch switching has the same problem — only one version is live at a time, and the install step has to re-run on each switch.
...
1. **Snapshot.** Extract that commit's `homegrown/` into `archived_skills/<sha>-hg/` (the project's convention for preserved skill states; create `archived_skills/` if it doesn't exist). Use `git archive`, not `git checkout`, so current `homegrown/` is never touched:
...
- **Multiple snapshots can coexist.** `bf4ae1f-hg/`, `cd44648-hg/`, `pre-X-hg/` all live under `archived_skills/`, alongside their respective install scripts (`install_<sha>_for_claude.sh`). The active `homegrown/` stays at the project root, separate from the archive. Disk is cheap; clarity is not.
...
for f in <sha>-MVL/SKILL.md "<sha>-MVL+/SKILL.md" <sha>-meta-loop/SKILL.md; do
  sed -i '' \
    ...
    -e 's|`explore`|`<sha>-explore`|g' \
    -e 's|`decompose`|`<sha>-decompose`|g' \
    -e 's|homegrown/MVL+/|homegrown/<sha>-MVL+/|g' \
    -e 's|homegrown/navigation/|homegrown/<sha>-navigation/|g' \
    "$f"
done
```
...
```bash
sed -i '' 's|homegrown/navigation/warmup/|homegrown/<sha>-navigation/warmup/|g' <sha>-navigation/SKILL.md
```
...
- `homegrown/protocols/` → `${HOME}/.claude/skills/<sha>-protocols/`
- `homegrown/<sha>-` → `${HOME}/.claude/skills/<sha>-`
...
git status --short -- homegrown/                                            # expect empty
...
sed -e "s|homegrown/protocols/|${HOME}/.claude/skills/<sha>-protocols/|g; s|homegrown/<sha>-|${HOME}/.claude/skills/<sha>-|g" \
  archived_skills/<sha>-hg/<sha>-MVL/SKILL.md | grep -nE 'homegrown|<sha>-protocols' | head
                                                                             # expect: only absolute install paths, no bare homegrown/
```
...
| meta-loop SKILL | `homegrown/MVL+/SKILL.md`, `homegrown/navigation/SKILL.md` | meta-loop loads these by path |
| navigation SKILL (~line 42) | `homegrown/navigation/warmup/navigator-refresh.md` | navigation loads this by path on `refresh_needed` |
| All runner SKILLs (rewritten at install time) | `homegrown/protocols/*` | Resolves to the renamed protocols install dir |
| All SKILLs (rewritten at install time) | `homegrown/<sha>-*` | Resolves cross-skill paths to absolute install dirs |

What does NOT need rewriting (and why):

- **Protocol files' `homegrown/...` references** — these are documentation prose (failure-mode descriptions, examples, cross-discipline pointers like `outcome_review.md`'s mention of `homegrown/contracts/alignment_control.md`). They're not load-at-runtime paths. The original install script doesn't rewrite them either.
...
- **The install script must apply its sed to with-refs skills too**, not just runners. Navigation has an executable `homegrown/...` path on line ~42 that needs the install-time rewrite. The original homegrown install script only sed-rewrites runners, but that's because the original disciplines don't reference `homegrown/...` paths — bf4ae1f's navigation does.
...
- **The `git archive | tar` extraction strips one path component** (`--strip-components=1`) so the snapshot folder's contents are `<sha>-hg/MVL/...` not `<sha>-hg/homegrown/MVL/...`. This matches the structure the rest of the recipe assumes.
