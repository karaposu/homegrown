...
**RESUME** — Pick up a saved inquiry across sessions OR between disciplines. Read `_state.md`, read each completed discipline's `Overall: PROCEED / FLAG / RE-RUN` verdict, route the loop accordingly (PROCEED to next / FLAG with shortfall + wait / RE-RUN with feedback + wait). Implemented as `cognitive_harness/protocols/resume.md`. **Not currently invoked by `/MVL` or `/MVL+`** — those auto-chain runners don't gate on per-discipline FLAG/RE-RUN waits (per their "pipeline runs continuously, checkpoints are informational not gates" rule). Available for manual-flow contexts and future runners that need telemetry-aware iteration resume. Reflect is structurally exempt (advisory output, not read for routing).
