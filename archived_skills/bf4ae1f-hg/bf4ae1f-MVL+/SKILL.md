...
# /bf4ae1f-MVL+ — The Extended Cognitive Loop
...
This is the extended form of the cognitive loop. `/bf4ae1f-mvl` (classic) runs only S → I → C. `/bf4ae1f-mvl+` adds Exploration (map territory) and Decomposition (partition complexity) to the first phase. Use `/bf4ae1f-mvl+` as the default for new inquiries; use `/bf4ae1f-mvl` classic for simple well-defined problems when speed matters.
...
3. Verify `flow-type: extended` in `[inquiry_path]/_state.md`. If the field is `classic` or absent, this inquiry belongs to `/bf4ae1f-mvl`, not `/bf4ae1f-mvl+` — flag to the user and stop.
...
|---|---|---|
| Exploration | `bf4ae1f-explore` | `exploration.md` |
| Sensemaking | `bf4ae1f-sense-making` | `sensemaking.md` |
| Decomposition | `bf4ae1f-decompose` | `decomposition.md` |
| Innovation | `bf4ae1f-innovate` | `innovation.md` |
| Critique | `bf4ae1f-td-critique` | `critique.md` |
...
```
If the user skips, move on. No gate. No requirement. Observations accumulate over time. When patterns emerge across multiple observations, the user can run `/bf4ae1f-MVL+ "review improvement observations and propose spec changes"` — the loop on the system's own feedback.
...
7. **Classic `/bf4ae1f-mvl` is UNCHANGED.** This command (`/bf4ae1f-mvl+`) is separate and coexists with classic. Existing classic inquiries resume with `/bf4ae1f-mvl`, not `/bf4ae1f-mvl+`. The `flow-type` field in `_state.md` distinguishes them.
