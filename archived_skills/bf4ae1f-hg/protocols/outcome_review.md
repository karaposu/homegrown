> **Loading note.** This file is loaded by a human, MVL/MVL+, materialization, navigation, or a future runner when a completed finding, branch, protocol edit, or materialized artifact has been used and needs after-use review. Read `homegrown/contracts/alignment_control.md` before executing this protocol. For full reviews or calibration-sensitive reviews, also read `enes/alignment_dynamics.md`. This protocol implements outcome-alignment insurance; it does not replace reflect, loop diagnose, navigation, materialization, or validation.
...
## Relationship to Alignment Dynamics

`enes/alignment_dynamics.md` explains the lifecycle that turns alignment feedback into future control improvement:
...
For full reviews or calibration-sensitive cases, also read:
...
Use the lightest sufficient route from `homegrown/contracts/alignment_control.md`.
...
Outcome review uses `homegrown/contracts/alignment_control.md` for layer, mode, record, and routing semantics.

### Alignment Dynamics

Outcome review uses `enes/alignment_dynamics.md` to understand evidence maturity and calibration in full or calibration-sensitive reviews.

Outcome review should not absorb alignment dynamics wholesale. It should preserve after-use and route-quality evidence, then route to navigation, loop diagnose, materialization, or protocol revision when the evidence justifies further work.
