...
# /bf4ae1f-meta-loop — Stateful Traversal Engine

The meta-loop is a controlled traversal over inquiry artifacts. It starts from a **seed plus context**, uses `/bf4ae1f-MVL+` as the probe that creates findings, uses `/bf4ae1f-navigation` as the eyes that map possible next moves, uses explicit selection to choose movement, and records cross-run memory in `_meta_state.md`.
...
2. Create the meta-loop folder:
   ...
   ```text
   devdocs/bf4ae1f-meta-loops/<YYYY-MM-DD_HH-MM__slugified_seed>/
   ```
...
Input is a `devdocs/bf4ae1f-meta-loops/[folder]/` path or an `_meta_state.md` path.
...
1. Load `homegrown/bf4ae1f-MVL+/SKILL.md`.
2. Run `/bf4ae1f-MVL+` on the active frontier.
...
If the active frontier is an incomplete MVL+ inquiry folder, resume it with `/bf4ae1f-MVL+ [folder]`.

### Phase 2 — See with Navigation

If `## Next Action` says to run Navigation:

1. Load `homegrown/bf4ae1f-navigation/SKILL.md`.
2. Run `/bf4ae1f-navigation [latest_inquiry_folder]`.
