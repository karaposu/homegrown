...
This is why `docs/what_is_meaningful_traversal.md` matters. Without meaningful traversal, the meta-loop cannot tell the difference between productive thinking and spinning.
...
Exploration also found that `docs/what_is_meaningful_traversal.md` already frames the orchestration of many MVL+ loops as the system-level problem. This supports the user's intuition that the meta-loop is more than a simple runner.
