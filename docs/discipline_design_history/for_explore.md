...
The **runtime specification** lives at `cognitive_harness/explore/references/explore.md`. The runtime spec is loaded by `cognitive_harness/explore/SKILL.md` at Step 0; this design-history file is NOT loaded at runtime.
...
- **Staged execution subsection** (removed from the runtime spec in the 2026-05-14 bloat-reframe). The `/staged-explore` runner orchestrates `/explore` at progressively finer resolutions. The runner's spec lives in its own file (`cognitive_harness/runners/staged_explore.md` when materialized); the runtime `/explore` spec no longer cross-references it. *Revival:* /explore's runtime behavior changes in a way that affects /staged-explore.
