...
ANd lets see if this makes sense?: a new top-level skill (skill name decided as part of Item 3's design /MVL+ — candidates include /continuous, /loop, /meta-mvl) exists at cognitive_harness/<name>/SKILL.md + references/; the skill orchestrates multiple /MVL+ runs with conditional /navigate invocation and meaningful-traversal criteria; a _meta_state.md schema (or equivalent) tracks cross-iteration state; the runner integrates Items 2 + 4 cleanly; v2 parallel-multi-head extension hooks are explicitly noted in the design.
