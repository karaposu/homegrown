...
HomeGrown Agent is designed for long autonomous runs where human review doesn't happen at every step. The six agents provide continuous monitoring, mode-awareness, and mode-switching intelligence that replaces the human-in-the-loop, allowing the system to self-correct, adapt its posture, or halt before compounding errors.

---

## The Three Dimensions

HomeGrown Agent operates in a three-dimensional space. This is the core architectural insight.
...
This is the thesis HomeGrown is built on: as AI capabilities converge, the differentiator shifts from raw ability to the methodology that guides it. Current tools have the ability. They lack the framework.
...
The pattern: current tools are **mode-blind and methodology-free**. They have capabilities across multiple layers but no structured philosophy for when and how to apply them. They operate in one posture regardless of what the situation demands. HomeGrown Agent provides the missing structure.
...
Each agent uses the HomeGrown slash commands that match its layer. The same commands humans use manually become the agents' tools in autonomous mode. Commands also cluster by mode — each mode has a natural affinity for certain commands.
...
HomeGrown Agent is an architecture, not a runtime. It defines *what* agents do, *how they coordinate*, and *which modes they operate in* — not the infrastructure that runs them. The six agents can be implemented on top of different agent runtime frameworks depending on the requirements.
...
HomeGrown Agent's value is in the alignment architecture. The runtime is interchangeable.

---

## Market Position

Current AI development tools optimize for **speed of generation** in a **single mode**. HomeGrown Agent optimizes for **correctness of generation** across **seven modes**. These serve different needs:
