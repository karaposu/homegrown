...
- `homegrown/MVL+/SKILL.md` — at inquiry creation, runs `Pre-Template Checks` that detect triggers; for each fired trigger, the runner consults this file's relevant sub-section and obligates the resulting `_branch.md` content.
- `homegrown/protocols/conclude.md` — at finding compilation, runs trigger detection on the finding's content + `_branch.md` declarations; for each fired trigger, the runner consults this file's relevant sub-section and obligates the finding to include the required friction.
...
`homegrown/MVL+/SKILL.md` — step 1 trigger-check + `_branch.md` template `## Layer Commitment` section.
...
`homegrown/MVL+/SKILL.md` — step 1 trigger-check + `_branch.md` template `## Synthesis Trigger` section.
...
Not yet inline. When materialized, expected to land in `homegrown/MVL+/SKILL.md` step 1 trigger-check + `_branch.md` template note (likely as part of an extended Pre-Template Checks block).
...
- **COULD (without gating):** *"Replace `homegrown/explore/SKILL.md` with the standard skeleton."*
- **COULD (with gating):** *"Replace `homegrown/explore/SKILL.md` with the standard skeleton. Depends-on: MUST item 'Confirm weak-reading interpretation.' This COULD is GATED — do not act until the MUST resolves."*
...
`homegrown/protocols/conclude.md` — `### COULD-vs-MUST dependency gating` sub-section + `### COULD` template line referencing the gating sub-section.
...
`homegrown/protocols/conclude.md` — `### Synthesis re-test enforcement` sub-section + `## Inherited Commitments Re-test` finding-template optional section.
...
Not yet exists. When materialized, expected to land here as a full sub-section (the canonical home), with cross-references from `homegrown/protocols/conclude.md` and `homegrown/MVL+/SKILL.md` as needed.
